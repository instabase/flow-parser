...
class TaskMissingError(Exception):
    def __init__(self, name):
        self.message = f"Missing task data for {name}"
        super(TaskMissingError, self).__init__(self.message)
    def __repr__(self):
        return self.message
...
class FileWriterError(Exception):
    def __init__(self, name):
        self.message = f"Cannot write file for {name}"
...
class APIKeyError(Exception):
    def __init__(self):
        self.message = "Instabase API key needs to be defined"
...
class APIResponseError(Exception):
    ...
    requests exception error that might happen during communication with Instabase API to simplify caller coding.
    """
